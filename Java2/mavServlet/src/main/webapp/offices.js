...
document.getElementById("bt_searchAll").onclick = searchAll;
document.getElementById("bt_searchNam").onclick = searchName;

function searchAll(e) {
    e.preventDefault();

    fetch('http://localhost:8080/mavServlet/api/offices')
            .then(response => response.json())
            .then(json => creaTabellaDaJson(json, "officeCode,city,phone", "tabOff", "tabResp", "#contenitore"))
}

function searchName(e) {
    e.preventDefault();
    let strCity = document.getElementById("searchUffName").value;

    fetch('http://localhost:8080/mavServlet/api/offices/searchCity?city=' + strCity)
            .then(response => response.json())
            .then(json => creaTabellaDaJson(json, "officeCode,city,phone", "tabOff", "tabResp", "#contenitore"))
